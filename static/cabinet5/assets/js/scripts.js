    ...
    if ($(elm).exists()) {
      $(elm).each(function () {
        ...
        var def = {
          responsive: true,
          autoWidth: false,
          dom: '<"row justify-between g-2"<"col-7 col-sm-6 text-left"f><"col-5 col-sm-6 text-right"<"datatable-filter"l>>><"datatable-wrap my-3 table-responsive"t><"row align-items-center"<"col-7 col-sm-12 col-md-9"p><"col-5 col-sm-12 col-md-3 text-left text-md-right"i>>',
          language: {
            search: "",
            searchPlaceholder: Vars['LngDataTable']['TypeSearch'],
            lengthMenu: "<span class='d-none d-sm-inline-block'>"+ Vars['LngDataTable']['Show'] +"</span><div class='form-control-select'> _MENU_ </div>",
            info: "_START_ -_END_ "+ Vars['LngDataTable']['of'] + " _TOTAL_",
            infoEmpty: Vars['LngDataTable']['NoRecordsFound'],
            infoFiltered: "( Total _MAX_  )",
            paginate: {
              "first": Vars['LngDataTable']['First'],
              "last": Vars['LngDataTable']['Last'],
              "next": Vars['LngDataTable']['next'],
              "previous": Vars['LngDataTable']['prev']
            }
          }
        },
            attr = opt ? extend(def, opt) : def;
